...
\end{figure}
We can only spot 5 eigenvalue lines, even though we know it should be 6 and the legend tells us there is 6 eigenvalues. The reason is obvious, eigenvalue 3 is hidden behind eigenvalue 4 since they are identical. We have a double degeneracy, i.e, the energy for state 3 and 4 are the same. 

\begin{figure}[H]
\centering
\includegraphics[width=100mm]{probability_FCI.png}
\caption{\label{fig:prob_FCI}}
\end{figure}

\subsection*{2C}
The single excited determinants ($|\Phi_i^a\rangle$) will not contribute to the exact eigenfunction since we require $p=\bar{p}$, i.e two particles with opposite spin shall always be at the same level. We will work with Configuration-Interaction Doubles (CID), but have in mind that it is the same as Configuration-Interaction Singles-Doubles (CISD). In general the doubly excited determinants can be written
\begin{equation}
...
\end{pmatrix}
\end{equation}
Because CID now corresponds to CISD, we get
\begin{equation}
...
Considering the low number of matrix elements, we get a quite good approximation of the groundstate energy using CID. 

\begin{figure}[H]
\centering
\includegraphics[width=100mm]{probability_CID.png}
\caption{\label{fig:prob_CID}}
\end{figure}
...
\end{equation}
where $E^{(0)}=\langle\Phi|\Phi\rangle$ is the groundstate energy and the other energies are expressed in the previous exercise. There are two way to compute this: We could use second quantization and many-body perturbation theory (MBPT) or we could use basic PT on matrix form. 
...
\subsection*{3G}
We can solve this given an initial condition, a $g$ and a $\sigma$. We are given the initial guess $t_i^{a(0)}=0$, and we can then solve this difference equation recursively. The first iteration gives
\begin{equation}
t_i^{a(1)}=-\frac{1}{2}g[2(\epsilon_i-\epsilon_a-1/2g)+\sigma]^{-1}
\end{equation}
and so on. we need to solve all the $t_i^{a(i)}$ matrix (for all indexes) before we go on to the next iteration $t_i^{a(i+1)}$. Further iterations are computed numerically, see Appendix B for script. For $\sigma=1$ we get:
\begin{figure}[H]
\centering
\includegraphics[width=100mm]{probability_CID.png}
\caption{\label{fig:prob_CID}}
